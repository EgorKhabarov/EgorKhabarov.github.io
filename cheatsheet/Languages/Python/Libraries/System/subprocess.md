...
| `subprocess.TimeoutExpired`     | Вызывается, если процесс не завершился в течение указанного времени<br>(используется с `timeout`) |
...
| Параметр             | Описание                                                                                                                               |
|----------------------|----------------------------------------------------------------------------------------------------------------------------------------|
| `stdin`              | Определяет источник стандартного ввода процесса<br>(например, `subprocess.PIPE`, `None`, или файловый объект)                          |
| `stdout`             | Определяет место, куда будет направлен стандартный вывод процесса<br>(например, `subprocess.PIPE`, `None`, или файловый объект)        |
| `stderr`             | Определяет место, куда будет направлен стандартный поток ошибок процесса<br>(например, `subprocess.PIPE`, `None`, или файловый объект) |
| `shell`              | Если `True`, команда будет выполнена через оболочку (shell)<br>Будьте осторожны, так как это может быть небезопасно                    |
| `cwd`                | Устанавливает текущий рабочий каталог для процесса                                                                                     |
| `env`                | Определяет переменные окружения для процесса                                                                                           |
| `universal_newlines` | Если `True`, ввод-вывод будет интерпретироваться как текст (строки),<br>а не байты                                                     |
| `timeout`            | Определяет максимальное время ожидания завершения процесса,<br>после чего будет вызвано исключение `TimeoutExpired`                    |
