Библиотека `operator` в Python предоставляет функции, которые соответствуют основным операциям, таким как арифметические, логические, сравнения и работа с атрибутами, элементами и срезами
Эти функции полезны для повышения читабельности и производительности кода, особенно при использовании в функциях высшего порядка, таких как `map()`, `filter()` и `reduce()`
