...
Чтобы использовать цвета в `curses`, необходимо сначала инициализировать цветовой режим с помощью `start_color()` и создать пары цветов
...
При завершении программы важно корректно выйти из режима `curses`, чтобы терминал вернулся в нормальный режим
Обычно для этого вызывается `curses.endwin()`, но использование `curses.wrapper()` автоматически обрабатывает завершение
