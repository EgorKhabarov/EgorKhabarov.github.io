...
| `.pyi`       | Файлы с аннотациями типов (типовыми подсказками)<br>Используются для статического анализа кода и проверки типов                                                          |
...
Эти файлы позволяют статическим анализаторам кода (например, `MyPy`) проверять корректность типов без выполнения самого кода
...
### .pyd
Динамически загружаемые библиотеки (`DLL`), написанные на `C` или `C++` и используемые как модули Python
...
### .pyx
Содержат исходный код на Cython, который представляет собой надстройку над Python, позволяющую компилировать код в `C` для повышения производительности
