...
Триггеры в SQLite работают для каждой строки (`FOR EACH ROW`), но не поддерживают уровень операций (`FOR EACH STATEMENT`)
- Отсутствие вложенных триггеров:
Если внутри триггера выполняется операция, которая вызывает другой триггер, SQLite запретит выполнение вложенных триггеров по умолчанию
- Оптимизация условий:
