...
```python
import sys,os,re,requests,time,arrow,html,urllib.parse,math,builtins
...
video=lambda length,speed=1:(lambda seconds:f'{seconds//3600}:{(seconds%3600)//60:02}:{seconds%60:02} (seconds={seconds})'.lstrip('0:'))(sum(x*60**i for i,x in enumerate(reversed(list(map(int,str(length).split(':'))))))//speed)
