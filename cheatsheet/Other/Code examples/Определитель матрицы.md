<p>```python@matrix.py
class Matrix:
    def <strong>init</strong>(self, matrix: list[list[int]]):
        self.matrix = matrix</p>
<pre><code>def get_minor(self, i: int, j: int) -&gt; list[list[int]]:
    result = [
        list(row[:j] + row[j+1:])
...
<p><code>python@test_matrix_2.py
