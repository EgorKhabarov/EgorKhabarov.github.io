...
**Узел** - это изменяющаяся часть IP
Каждое устройство имеет свой уникальный адрес в сети, он называется узлом

Маску принято записывать двумя способами: `префиксным` и `десятичным`
Например, маска частной подсети `A` выглядит в десятичной записи как `255.0.0.0`, но не всегда удобно пользоваться десятичной записью при составлении схемы сети
