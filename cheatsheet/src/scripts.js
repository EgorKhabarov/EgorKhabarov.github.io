...
fts_input.addEventListener("input", debounce(async (e) => {
    ...
    if (results === null) {
        folder_search_list.style.display = "flex";
        folder_search_list.style.alignItems = "center";
        folder_search_list.style.justifyContent = "center";
        folder_search_list.style.color = "red";
        folder_search_list.textContent = "An error occurred";
        fts_input.style.border = "1px solid red";
        return;
    }
    fts_input.style.border = null;
    folder_search_list.scrollTo(0, 0);
    generateButtons(buildTree(results.map(r => r.f)), folder_search_list, false);
    folder_search_list.querySelectorAll('[data-state="closed"]').forEach(e => {
        e.setAttribute("data-state", "open");
        e.nextElementSibling.classList.remove("hidden");
    })
    folder_search_list.querySelectorAll(".file").forEach(e => {
        e.onclick = async function(event) {
            ...
            page_search_input.value = search_query.trim();
            setTimeout(function() {
                page_search_input.dispatchEvent(new Event("input"));
            }, 300);
        }
    });
    folder_search_list.querySelectorAll(".file").forEach(e => {
        const url = e.getAttribute("data-vpath");
        const e2 = folder_list.querySelector(`[data-vpath="${url}"]`);
        if (e && e2) {
            e.firstChild.style.color = e2.firstChild.style.color;
        }
    });

    if (search_query && !folder_search_list.innerHTML) {
        folder_search_list.style.display = "flex";
        folder_search_list.style.alignItems = "center";
        folder_search_list.style.justifyContent = "center";
        folder_search_list.style.color = null;
        folder_search_list.textContent = "No results";
    } else if (folder_search_list.innerHTML !== "No results") {
        folder_search_list.style.alignItems = null;
        folder_search_list.style.justifyContent = null;
    } else {
        folder_search_list.style.display = null;
        folder_search_list.style.alignItems = null;
        folder_search_list.style.justifyContent = null;
        folder_search_list.style.color = null;
        folder_search_list.innerHTML = "";
    }
}, 500));
function closeSearch() {
    folder_list.style.display = "block";
    folder_search_list.style.display = "none";
    folder_search_list.style.display = null;
    folder_search_list.style.alignItems = null;
    folder_search_list.style.justifyContent = null;
    folder_search_list.innerHTML = "";
...
function parseSearchQuery(query, inputEl) {
    ...
    if (
        ...
        try {
            ...
            page_search_result_count.style.color = null;
            page_search_result_count.textContent = "0 results";
            return regex;
        } catch (e) {
            // RegExp syntax error
            inputEl.style.border = "1px solid red";
            page_search_result_count.style.color = "red";
            page_search_result_count.textContent = "Bad pattern";
            return null;
        }
    }
...
function highlightText(rawQuery, cheatsheetField, inputEl) {
    ...
    page_search_result_count.style.color = null;
    page_search_result_count.textContent = `1/${allRanges.length}`;
...
function setActiveHighlight(index) {
    ...
    page_search_result_count.style.color = null;
    page_search_result_count.textContent = `${activeIndex+1}/${highlightRanges.length}`;
}
...
let last_search = "";
page_search_input.addEventListener("input", () => {
    if (page_search_input.value !== "") {
        console.log(`"${page_search_input.value}"`);
        last_search = page_search_input.value;
    }
    highlightText(page_search_input.value, nowCheatsheetField, page_search_input);
    nextHighlight();
});
function closePageSearch() {
    page_search_input.value = "";
    page_search_input.dispatchEvent(new Event("input"));
...
function renderBreadcrumbs(url) {
    ...
    pathArray.forEach((part, index) => {
        ...
        breadcrumbs_content.appendChild(span);
        if (index < pathArray.length - 1) {
            const sep = document.createElement("span");
            sep.textContent = "/";
            sep.className = "crumb_separator";
            breadcrumbs_content.appendChild(sep);
        }
    });
...
function changeActiveButton(element) {
    for (el of folder_list.getElementsByClassName("tree_item_active")) {
...
function closeAllKeyButtons() {
    folder_list.querySelectorAll('[data-state="open"]').forEach(e => {
        e.setAttribute("data-state", "closed");
        e.nextElementSibling.classList.add("hidden");
    })
}
function displayKeyButton(url) {
    ...
    while (pathArray.length) {
        ...
        const parent_directory = folder_list.querySelector(`[data-kpath="${kpath}"]`).nextElementSibling;
        parent_directory.previousElementSibling.setAttribute("data-state", "open");
        parent_directory.classList.remove("hidden");
        pathArray.pop();
    }
    let button = folder_list.querySelector(`[data-kpath="${url}"]`);
...
function displayValueButton(url, scrollIntoActive = true) {
    ...
    let button = folder_list.querySelector(`[data-vpath="${url}"]`);
    if (scrollIntoActive) {
        button?.scrollIntoView({block: "center"});
    }
    return button;
...
function init() {  // load_folder_list
    fetch("src/index.json")
        ...
        .then(async (json_data) => {
            generateButtons(json_data, folder_list);

            folder_list.querySelectorAll(".file").forEach(e => {
                e.onclick = async function(event) {
                    urlArgs.setAnchor(null);
                    const vpath = e.getAttribute("data-vpath")
            ...
            if (s_param) {
                fts_input.value = s_param;
                fts_input.dispatchEvent(new Event("input"));
...
sidebar_toggle.addEventListener("click", () => leftDrawer.toggle("--sidebar-left-display"));
toc_toggle.addEventListener("click", () => rightDrawer.toggle("--sidebar-right-display"));
...
function toggleSettings(show, page) {
    isSettingsOpen = show;
    if (show) {
        settings_modal.classList.add("active");
        ...
        settings_modal.classList.remove("active");
...
settings_btn.addEventListener("click", () => toggleSettings(true));
close_settings.addEventListener("click", () => toggleSettings(false));
settings_backdrop.addEventListener("click", () => toggleSettings(false));
...
page_search_close.addEventListener("click", closePageSearch);
...
setupClearButton(fts_input, fts_clear_button);
setupClearButton(page_search_input, page_search_clear_button);
...
document.addEventListener("keydown", (e) => {
    ...
    if (e.ctrlKey && e.code === "KeyF") {
        ...
        page_search_input.focus();
        if (selection && !selection.includes("\n")) {
            page_search_input.value = selection;
            page_search_input.dispatchEvent(new Event("input"));
        } else if (last_search.length > 1) {
            page_search_input.value = last_search;
            page_search_input.dispatchEvent(new Event("input"));
            page_search_input.select();
        }
        return;
    }
    ...
    if (e.ctrlKey && e.shiftKey && e.code === "Equal") {
        e.preventDefault();
        folder_list.querySelectorAll('[data-state="closed"]').forEach(e => {
            e.setAttribute("data-state", "open");
            e.nextElementSibling.classList.remove("hidden");
        });
        return;
