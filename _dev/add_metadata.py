...
def add_metadata(path_: str, metadata: dict) -> None:
    ...
    print(f"\x1b[33mNow metadata\x1b[0m {json.dumps(result_metadata, indent=4, ensure_ascii=False)}")
    ...
    print(f"\x1b[32mNew metadata\x1b[0m {json.dumps(result_metadata, indent=4, ensure_ascii=False)}")
    if input('\x1b[31mTo approve, write "y" or "yes": \x1b[0m') in ("y", "yes"):
        path_dict[path_] = yaml.safe_dump(result_metadata, sort_keys=False, allow_unicode=True, indent=4)
        ...
        print(f"\x1b[32mMetadata updated successfully\x1b[0m")
